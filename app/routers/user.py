...
def obtener_usuarios(
    data_base: Session = Depends(get_db), current_user: User = Depends(get_current_user)
) -> List[ShowUser]:
...
def obtener_usuario(
    ...
    :return:
    """
    usuario: Union[dict[str, str], Any] = user.obtener_usuario(
        user_id, data_base=data_base
    )
    return usuario
...
def actualizar_user(
    user_id: int,
    usuario_actualizado: UpdateUser,
    data_base: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Actualiza un usuario"""
    usuario = user.actualizar_usuario(
        user_id, data_base=data_base, usuario_actualizado=usuario_actualizado
    )
    return usuario
